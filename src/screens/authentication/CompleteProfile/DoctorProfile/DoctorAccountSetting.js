...
export default function DoctorAccountSetting() {
  ...
  function isFormValid() {
    ...
    const isOfflineVisitValid = !offline_visit.available || (
      ...
      offline_visit.from.trim() !== "" &&
      offline_visit.to.trim() !== ""
    );
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBlock: 5 }}>
              <View>
                ...
                <DateTimeInput
                  disabled={!formValues.consultation_type.video_call.available}
                  mode="date"
                  onChangeValue={(val) => updateConsultationType('video_call', 'from', new Date(val).toISOString().split("T")[0])}
                />
              </View>
              <View>
                ...
                <DateTimeInput
                  disabled={!formValues.consultation_type.video_call.available}
                  mode="date"
                  onChangeValue={(val) => updateConsultationType('video_call', 'to', new Date(val).toISOString().split("T")[0])}
                />
              </View>
            </View>
          </View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBlock: 5 }}>
              <View>
                ...
                <DateTimeInput
                  disabled={!formValues.consultation_type.offline_visit.available}
                  mode="date"
                  onChangeValue={(val) => updateConsultationType('offline_visit', 'from', new Date(val).toISOString().split("T")[0])}
                />
              </View>
              <View>
                <Text style={[common_styles.medium_text_normal_weight, { marginBottom: 3 }]}>To Date</Text>
                <DateTimeInput
                  disabled={!formValues.consultation_type.offline_visit.available}
                  mode="date"
                  onChangeValue={(val) => updateConsultationType('offline_visit', 'to', new Date(val).toISOString().split("T")[0])}
                />
              </View>
            </View>
          </View>
        ...
        <View style={{ marginBlock: 15 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Switch
              ...
              trackColor={{ false: '#767577', true: colorTheme.primaryColor }}
              ios_backgroundColor="#3e3e3e"
              onValueChange={() => updateFormValues('instant_chat_option', !formValues.instant_chat_option)}
